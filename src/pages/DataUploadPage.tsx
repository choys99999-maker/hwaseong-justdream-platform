...
import { AlertCircle, FileSpreadsheet, RotateCcw, Upload } from 'lucide-react';
  ...
  function processFile(file: File) {
    ...
    reader.onload = (e) => {
      try {
        ...
        // Build column index keeping original position for correct data alignment
    ...
    <div className="space-y-6">
      ...
      {step === 'uploading' && (
        <section className="flex flex-col items-center gap-3 rounded-xl border border-slate-200 bg-white p-10 text-center">
          ...
          <p className="text-sm font-medium text-slate-800">파일을 읽는 중...</p>
          <p className="text-xs text-slate-400">열 구조를 분석하고 있습니다.</p>
        </section>
      )}
          ...
          <div>
            <h3 className="text-sm font-semibold text-slate-700">
              ...
              <span className="font-normal text-slate-400">
                (최대 10행 · 총 {preview.rows.length}행 표시)
              </span>
            </h3>
