...
const AVAILABILITY_OPTIONS: { value: SiteAvailability; label: string; icon: typeof PackageCheck; className: string }[] = [
...
/**
 * 빠른 현황 입력. 시민 화면(`/`, `/site/:id`)의 "지금 상태"가 바로 이 화면이 저장하는 값이다.
 * 현장 담당자가 전체 관리자 화면을 쓰지 않고 거점·상태 두 번의 선택 + 저장만으로
 * 1분 이내 끝낼 수 있게 화면을 최소화했다.
 */
export default function QuickSiteStatusPage() {
  ...
  async function handleSave() {
    ...
    setSaveError(null);
    try {
      await upsertSiteQuickStatus({ siteId, availability, focusItem: focusItem.trim() || undefined, note: note.trim() || undefined });
  ...
  return (
    <div className="mx-auto w-full max-w-[640px] space-y-5">
      ...
      <div className="space-y-4 rounded-xl border border-slate-200 bg-white p-5">
        <div>
          ...
          {currentStatus && (
            <p className="mt-1.5 text-xs text-slate-400">
              ...
              · {AVAILABILITY_OPTIONS.find((o) => o.value === currentStatus.availability)?.label}
        ...
        <div>
          <p className="mb-1.5 text-sm font-semibold text-slate-700">2. 지금 상태</p>
