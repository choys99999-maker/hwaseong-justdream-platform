...
- Name: demo-mode.spec.ts >> 시연 모드 — 일반 접근에서는 노출되지 않는다 >> 시민 홈에는 작은 진입 버튼만 있고, 역할 전환 UI는 기본적으로 숨어 있다
- Location: e2e/demo-mode.spec.ts:16:3
...
Locator: getByRole('button', { name: '시연 모드' })
Expected: visible
Timeout: 5000ms
